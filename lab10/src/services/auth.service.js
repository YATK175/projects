...
const createDbAuthService = (db) => ({
  ...
  async register({ email, password }) {
    ...
    const result = await db.insert(usersTable).values({ email: normalizedEmail, password: hash });
...
export const createAuthService = ({ db } = {}) =>
  hasDrizzleLikeDb(db) ? createDbAuthService(db) : createJsonAuthService();
