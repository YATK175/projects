...
const getSortedJsonFiles = async () => {
  await ensureDir(ITEMS_DIR);
  const files = await fs.readdir(ITEMS_DIR);
...
export const itemsRepository = {
  ...
  async *streamAll(author) {
    const files = await getSortedJsonFiles();

    for (const file of files) {
      const item = await readJsonFile(path.join(ITEMS_DIR, file));

      if (!author || item.author.toLowerCase().includes(author.toLowerCase())) {
        yield item;
      }
    }
  },
