...
const fetchWithRetry = async (url, logger) => {
  ...
  for (let attempt = 0; attempt <= retryDelays.length; attempt += 1) {
    try {
      ...
      logger.warn({ err: error, attempt: attempt + 1 }, 'External service request failed. Retrying.');
...
export const externalReferenceService = {
  ...
  async getGenreByName(baseUrl, genreName, logger, redis) {
    try {
      ...
      logger.warn({ err: error }, 'External genre service is unavailable. Returning degraded response.');
      return null;
    }
  },
};
